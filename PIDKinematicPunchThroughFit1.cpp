...
void PIDKinematicPunchThroughFit1()
{
  ...
  ofstream FileOut;
  FileOut.open(Form("calibrations/FitParameters_%s", FileOutTag.c_str()));
  FileOut << setw(5) <<"*telnum" <<"  "<< setw(10) << "csinum" <<"  "<< setw(10) << "Par0" <<"  "<<setw(10)<<"Par1\n";

  //---- Open data files --------------------------
  Int_t NFiles=10;
  std::string * FileIn_name[NFiles];
  FileIn_name[0] = new std::string(Form("calibrations/HiRA_CsI_DEELowEnergy_Z%02d_A%02d.dat", Z, A));
  FileIn_name[1] = new std::string(Form("calibrations/HiRA_CsI_DEEMidEnergy_Z%02d_A%02d.dat", Z, A));
  FileIn_name[2] = new std::string("calibrations/Corrected_HiRA_CsIKinimatics_2013_2037_40Ca39AMeV_gain 200.dat");
  FileIn_name[3] = new std::string("calibrations/Corrected_HiRA_CsIKinimatics_2312_2312_40Ca56AMeV_gain 170.dat");
  FileIn_name[4] = new std::string("calibrations/Corrected_HiRA_CsIKinimatics_4034_4037_48Ca28AMeV_gain 170.dat");
  FileIn_name[5] = new std::string("calibrations/Corrected_HiRA_CsIKinimatics_4332_4332_48Ca56AMeV_gain 170.dat");
  FileIn_name[6] = new std::string("calibrations/Corrected_HiRA_CsIKinimatics_4577_4584_48Ca28AMeV_gain 170.dat");
  FileIn_name[7] = new std::string("calibrations/Corrected_HiRA_CsIKinimatics_4585_4589_48Ca56AMeV_gain 170.dat");
  FileIn_name[8] = new std::string(Form("calibrations/HiRA_CsI_PunchThrough_Z%02d_A%02d.dat", Z, A));
  FileIn_name[9] = new std::string(Form("calibrations/WMUdata_Z%02d_A%02d.dat", Z, A));
...
//    TColor * color = new TColor(ci, 0.1*FileNum, 0.2*FileNum, 0.3*FileNum);
      ...
      for(int j=0; j<CsINum; j++)
      {
        if(Npoints[i][j][FileNum]>0)
        {
         ...
         // DataSet[i][j][FileNum] -> SetMarkerStyle(20+FileNum);
          ...
          {
            DataSet[i][j][FileNum] -> SetMarkerStyle(20+ci);
            DataSet[i][j][FileNum] -> SetMarkerColor(1);
            DataSet[i][j][FileNum] -> SetLineColor(1);
          }
          else
          {
  ...
  for(int i=0; i<TelNum; i++)
  {
    for(int j=0; j<CsINum; j++)
    {
      legend[i][j] = new TLegend(0.45, 0.15, 0.90, 0.42);
...
//       gPad->SetLogy(1);
 ...
 //     DataGraph[i][j]->Fit(fit_func,"W RC+");
   ...
   //  getchar();
      ...
      c1->Print(Form("FitResult/CompareWithWMU/PID_Kinematics_PunchThrough_Tel%02d_CsI%02d_%s.png",i, j, FileOutTag.c_str()));
    }
  }
