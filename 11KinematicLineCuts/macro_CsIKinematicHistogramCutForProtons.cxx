void macro_CsIKinematicHistogramCutForProtons()
{
  const int FirstRun=4332;
  const int LastRun=4332;
  const double E_projectile=56; //MeV
  const int Z_beam=20; 
  const int A_beam=48; 
  
  std::string path_to_file (Form("output/NewGeo_HiRA_KinematicPlots_%4d_%4d_%2dCa+1H@%.0fAMeV.root",FirstRun,LastRun,A_beam,E_projectile));
  TFile * FileIn = new TFile(path_to_file.c_str());

  TFile * fileout = new TFile(Form("output/NewGeo_HiRA_KinematicPlotsCut_%4d_%4d_%2dCa+1H@%.0fAMeV.root",FirstRun,LastRun,A_beam,E_projectile), "UPDATE");
