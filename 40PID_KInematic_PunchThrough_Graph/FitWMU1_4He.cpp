...
void FitWMU1_4He()
{
   ...
   ifstream FileInLowEnergy(Form("datfile/HiRA_CsI_DEELowEnergy_Z%02d_A%02d.dat", Z, A));
   ifstream FileInMidEnergy(Form("datfile/HiRA_CsI_DEEMidEnergy_Z%02d_A%02d.dat", Z, A));
   ifstream FileInWMU(Form("datfile/WMUdata_Z%02d_A%02d.dat", Z, A));
   
   ofstream FileOut;
   FileOut.open(Form("figures/Fitparameters_%s", FileOutTag.c_str()));
   ...
   for(int i=0; i<TELNUM; i++)
   {
     for(int j=0; j<CSINUM; j++)
     {
       ...
       data_low[i][j]->SetMarkerStyle(20);  // open circle
       ...
       data_WMU[i][j]->SetMarkerStyle(24);  // open circle
       ...
       //--- fit low energy data and mid energy data ----
       Fit_data_total[i][j]= new TMultiGraph();
       Fit_data_total[i][j]->Add(data_low[i][j]);
   ...
   for(int i=0; i<TELNUM; i++)
   {
     for(int j=0; j<CSINUM; j++)
     {
       ...
       c1->Print(Form("figures/PID_Kinematics_PunchThrough_Tel%02d_CsI%02d_%s.png",i, j, FileOutTag.c_str())); 
