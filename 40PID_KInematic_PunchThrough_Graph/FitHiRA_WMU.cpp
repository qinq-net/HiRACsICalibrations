...
void FitHiRA_WMU()
{
   ...
   std::string FileOutTag2("FullEnergy");
   ...
   for(int i=0; i<TELNUM; i++)
   {
    ...
    //   data_HiRA_deuteron[i][j]->SetMarkerSize(1.2);
     ...
     //  data_total[i][j]->Add(data_HiRA_4He[i][j]);      
       ...
       legend[i][j]->AddEntry(data_HiRA_proton1[i][j], "HiRA_proton", "P");
       legend[i][j]->AddEntry(data_WMU_proton[i][j], "WMU_proton", "P");
       legend[i][j]->AddEntry(data_HiRA_deuteron1[i][j], "HiRA_deuteron", "P");
       legend[i][j]->AddEntry(data_WMU_deuteron[i][j], "WMU_deuteron", "P");
       legend[i][j]->AddEntry(data_HiRA_4He1[i][j], "HiRA_4He", "P");             
