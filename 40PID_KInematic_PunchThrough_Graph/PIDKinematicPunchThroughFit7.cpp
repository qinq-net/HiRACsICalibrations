...
void PIDKinematicPunchThroughFit7()
{
 
  std::string FileOutTag("WMUdeuteron");
  ...
  FileIn_name[0]  = new std::string(Form("datfile/HiRA_CsI_DEELowEnergy_Z%02d_A%02d.dat", 1, 2));
  FileIn_name[1] = new std::string(Form("datfile/HiRA_CsI_DEEMidEnergy_Z%02d_A%02d.dat", 1, 2));
  FileIn_name[2] = new std::string(Form("datfile/HiRA_CsI_PunchThrough_Z%02d_A%02d.dat", 1, 2));
...
//    TColor * color = new TColor(ci, 0.1*FileNum, 0.2*FileNum, 0.3*FileNum);
  ...
  for(int i=0; i<TelNum; i++)
  {
    for(int j=0; j<CsINum; j++)
    {
      legend[i][j] = new TLegend(0.45, 0.2, 0.90, 0.50);
  ...
  //----------------- Add WMUdata to the graphs ----------------------
  double Voltage[TelNum][CsINum][10];
  double errVoltage[TelNum][CsINum][10];
  double energy[TelNum][CsINum][10];
  double errenergy[TelNum][CsINum][10];
  int datanum[12][4];
  ...
  ifstream FileInWMU(Form("datfile/WMUdata_Z%02d_A%02d", Z, A));
  while(FileInWMU.is_open() && !FileInWMU.eof())
  {
    ...
    LineStream >> telnum >> csinum;
    LineStream >> Voltage[telnum][csinum][datanum[telnum][csinum]] >> errVoltage[telnum][csinum][datanum[telnum][csinum]] >> energy[telnum][csinum][datanum[telnum][csinum]] >> errenergy[telnum][csinum][datanum[telnum][csinum]] ;
    
    datanum[telnum][csinum]++;
    
  }
   
  TGraphErrors * WMUdata[TelNum][CsINum];
  for(int i=0; i<TelNum; i++)
  {
    for(int j=0; j<CsINum; j++)
    {
      if(datanum[i][j]==0) continue;
      WMUdata[i][j] = new TGraphErrors(datanum[i][j], energy[i][j], Voltage[i][j], errenergy[i][j], errVoltage[i][j]);
      WMUdata[i][j]->SetMarkerStyle(24);
      WMUdata[i][j]->SetMarkerColor(kBlue);
      legend[i][j]->AddEntry(WMUdata[i][j],"WMUdata","P");
    }
  }
  ...
  for(int i=0; i<TelNum; i++)
  {
    for(int j=0; j<CsINum; j++)
    {
      ...
      TF1 * fit_func=new TF1(Form("fit_func%02d_%02d", i,j),"[0]*x^{[1]}",0.1, 270);
