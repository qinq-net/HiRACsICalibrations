...
double HornFit(double *x, double *par)
{
  int A = Int_t(x[1])%100;
  int Z = Int_t(x[1])/100;
  double consterm = par[5]*A*pow(Z,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(lineterm/consterm);
  double light = par[3]+par[4]*(x[0]-logterm);
  return light;
}

///////////////////////////////////////////////////////
///  definition of fitting function of 3He
double fit_3he(double *x, double *par)
{
  double consterm = par[5]*3*pow(2,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(lineterm/consterm);
  double light = par[3]+par[4]*(x[0]-logterm);
  return light;
...
double fit_4he(double *x, double *par)
{
  double consterm = par[5]*4*pow(2,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(lineterm/consterm);
  double light = par[3]+par[4]*(x[0]-logterm);
  return light;
...
double fit_6Li(double *x, double *par)
{
  double consterm = par[5]*6*pow(3,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(lineterm/consterm);
  double light = par[3]+par[4]*(x[0]-logterm);
  return light;
...
double fit_7Li(double *x, double *par)
{
  double consterm = par[5]*7*pow(3,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(lineterm/consterm);
  double light = par[3]+par[4]*(x[0]-logterm);
  return light;
...
double fit_8Li(double *x, double *par)
{
  double consterm = par[5]*8*pow(3,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(lineterm/consterm);
  double light = par[3]+par[4]*(x[0]-logterm);
  return light;
...
double fit_7Be(double *x, double *par)
{
  double consterm = par[5]*7*pow(4,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(lineterm/consterm);
  double light = par[3]+par[4]*(x[0]-logterm);
  return light;
...
double fit_9Be(double *x, double *par)
{
  double consterm = par[5]*9*pow(4,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(lineterm/consterm);
  double light = par[3]+par[4]*(x[0]-logterm);
  return light;
...
void PDTFit1()
{
    ...
    ************************************************************************************************************/
     ...
     double CsIV_deuteron[TELNUM][CSINUM][NFiles_deuteron][30];
     double err_CsIV_deuteron[TELNUM][CSINUM][NFiles_deuteron][30];
     double Energy_deuteron[TELNUM][CSINUM][NFiles_deuteron][30];
     double err_Energy_deuteron[TELNUM][CSINUM][NFiles_deuteron][30];
     double ZA_deuteron[TELNUM][CSINUM][NFiles_deuteron][30];
     ...
     ************************************************************************************************************/
     ...
     double CsIV_triton[TELNUM][CSINUM][NFiles_triton][30];
     double err_CsIV_triton[TELNUM][CSINUM][NFiles_triton][30];
     double Energy_triton[TELNUM][CSINUM][NFiles_triton][30];
     double err_Energy_triton[TELNUM][CSINUM][NFiles_triton][30];
     double ZA_triton[TELNUM][CSINUM][NFiles_triton][30];
     ...
     for(int FileNum=0; FileNum<NFiles_triton; FileNum++)
     {
       ...
       FileIn_triton.close();
            ...
            ************************************************************************************************************/
            ...
            double CsIV_4He[TELNUM][CSINUM][NFiles_4He][30];
            double err_CsIV_4He[TELNUM][CSINUM][NFiles_4He][30];
            double Energy_4He[TELNUM][CSINUM][NFiles_4He][30];
            double err_Energy_4He[TELNUM][CSINUM][NFiles_4He][30];
            double ZA_4He[TELNUM][CSINUM][NFiles_4He][30];
            ...
            for(int FileNum=0; FileNum<NFiles_4He; FileNum++)
            {
              ...
              while(!FileIn_4He.eof())
              {
                ...
                ZA_4He[telnum][csinum][FileNum][NPoints_4He[telnum][csinum][FileNum]]= 204;
              ...
              for(int i=0; i<TELNUM; i++)
              {
                for(int j=0; j<CSINUM; j++)
                {
                  if(NPoints_4He[i][j][FileNum]!=0)
                  {
                    ...
                    Data4He[i][j][FileNum]->SetMarkerColor(4); // blue marker for 4Hes
                    Data4He[i][j][FileNum]->SetLineColor(4);
